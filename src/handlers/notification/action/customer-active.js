import { EMAIL_TEMPLATE_CUSTOMER_ACTIVE } from '~/handlers/template/type';
...
export default ({ emailClient, htmlTemplate, cache, models }) => ({
  pushNotif: async ({ sender, name, data }) => {
    const templateEngine = htmlTemplate.factory(EMAIL_TEMPLATE_CUSTOMER_ACTIVE);
    ...
    // return emailClient.sendMail(sender, 'ESPP - Active account', emailBody);
  },
});
