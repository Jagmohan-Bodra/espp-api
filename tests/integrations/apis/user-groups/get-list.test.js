...
describe('Test GET://v1/user-groups', () => {
  ...
  beforeAll(async () => {
    await db.collection('user_groups').insertMany(fakeUserGroups);
  ...
  describe('Should return 200', () => {
    it('Should success without any filter', async () => {
      ...
      // expect(res.body.data[0]).toMatchObject({ name: 'name0' });
    ...
    it('Should success and ignore all invalid filter', async () => {
      const res = await request(server)
        .get(`/v1/user-groups?wrongfield[w]=1`)
        .set('Authorization', 'Bearer ' + token);
    ...
    it('Should success when filter with name', async () => {
      const res = await request(server)
        .get(`/v1/user-groups?name[regex]=name3`)
        .set('Authorization', 'Bearer ' + token);
    ...
    it('Should success when apply pagination', async () => {
      const res = await request(server)
        .get(`/v1/user-groups?meta[pageSize]=3&meta[page]=2`)
        .set('Authorization', 'Bearer ' + token);
    ...
    it('Should success when apply sort', async () => {
      const res = await request(server)
        .get(`/v1/user-groups?meta[pageSize]=3&meta[page]=2&meta[sort][]=-name`)
        .set('Authorization', 'Bearer ' + token);
  ...
  describe('Should return 401', () => {
    it('Should be failed when token is not set', async () => {
      const res = await request(server).get(`/v1/user-groups`);
