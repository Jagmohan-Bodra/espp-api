...
describe('Test GET://v1/tags', () => {
  ...
  beforeAll(async () => {
    await db.collection('tags').insertMany(fakeUserGroups);
  ...
  describe('Should return 200', () => {
    ...
    it('Should success when apply pagination', async () => {
      const res = await request(server)
        .get(`/v1/tags?meta[pageSize]=3&meta[page]=2`)
        .set('Authorization', 'Bearer ' + token);
    ...
    it('Should success when apply sort', async () => {
      const res = await request(server)
        .get(`/v1/tags?meta[pageSize]=3&meta[page]=2&meta[sort][]=-name`)
        .set('Authorization', 'Bearer ' + token);
  ...
  describe('Should return 401', () => {
    it('Should be failed when token is not set', async () => {
      const res = await request(server).get(`/v1/tags`);
