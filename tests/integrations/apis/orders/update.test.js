...
describe('Test GET: //v1/orders/:orderId', () => {
  let token;
  let customerId;
  let validData;
  let productIds;
  ...
  describe('Return status 400:', () => {
    ...
    it('Should be failed when status is invalid', async () => {
      const res = await request(server)
        .put(`/v1/orders/${orderId}`)
        .set('Authorization', 'Bearer ' + token)
        .send({ status: 'invalid' });
