...
describe('Test GET //v1/customer-internal-note/:customerId/:internalNoteId', () => {
  let token;
  let customerId;
  let userId;
  let internalNoteId;
  ...
  const userGroupData = {
    name: 'Zadmin',
    roles: [ROLES.CUSTOMER_INTERNALNOTE_GET],
  };
  const userData = {
    name: 'Mr. Admin',
    phone: '0923456789',
  ...
  describe('Return status 200', () => {
    it('Should be successful', async () => {
      const res = await request(server)
        .get(`/v1/customer-internal-note/${customerId}/${internalNoteId}`)
  ...
  describe('Return status 400', () => {
    it('Should response BadRequest when customerId is invalid', async () => {
      const res = await request(server)
        .get(`/v1/customer-internal-note/invalid-customerId/${internalNoteId}`)
        .set('Authorization', 'Bearer ' + token);
    ...
    it('Should response BadRequest when internalNoteId is invalid', async () => {
      const res = await request(server)
        .get(`/v1/customer-internal-note/${customerId}/invalid-internalNoteId`)
        .set('Authorization', 'Bearer ' + token);
  ...
  describe('Return status 404', () => {
    it('Should failed when customerId is not found', async () => {
      const res = await request(server)
        .get(`/v1/customer-internal-note/012345678912/${internalNoteId}`)
        .set('Authorization', 'Bearer ' + token);
    ...
    it('Should failed when internalNoteId is not found', async () => {
      const res = await request(server)
        .get(`/v1/customer-internal-note/${customerId}/012345678912`)
        .set('Authorization', 'Bearer ' + token);
