...
describe('Test GET //v1/post-categories/:id', () => {
  let token;
  let validId;
  ...
  describe('Return status 200', () => {
    it('Should be successful when "id" is exist', async () => {
      const res = await request(server)
        .get(`/v1/post-categories/${validId}`)
  ...
  describe('Return status 404', () => {
    it('Should failed when "id" is not exist', async () => {
      const res = await request(server)
        .get(`/v1/post-categories/${notExistsValidId}`)
        .set('Authorization', 'Bearer ' + token);
  ...
  describe('Should return 401', () => {
    it('Should be failed when token is not set', async () => {
      const res = await request(server).get(`/v1/post-categories/${validId}`);
