...
describe('PUT: //v1/posts/:id', () => {
  ...
  describe('Test Authentication handler (401)', () => {
    it('Should create failed when token is not set', async () => {
      const res = await request(server).put(`/v1/posts/${validId}`);
  ...
  describe('Test Validation Handler (400)', () => {
    ...
    it('Should response BadRequest when is Invalid', async () => {
      const res = await request(server)
        .put(`/v1/posts/${validId}`)
        .set('Authorization', 'Bearer ' + token)
        .send({ ...fakeData, postCategoryId: ['notObjectId'] });
      expect(res.status).toBe(400);
    });
  });
  ...
  describe('Test Resource Not found handler (404)', () => {
    ...
    it('Should response BadRequest when "postCategoryId" is not exist', async () => {
      const res = await request(server)
        .put(`/v1/posts/${validId}`)
        .set('Authorization', 'Bearer ' + token)
        .send({ ...fakeData, postCategoryId: [notExistsValidId] });
      expect(res.status).toBe(404);
    });
  });
});
