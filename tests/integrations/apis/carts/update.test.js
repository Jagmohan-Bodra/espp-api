...
describe('Test PUT://v1/carts/:cartId', () => {
  let token;
  let customerId;
  let cartData;
  let productId;
  ...
  describe('Return status 400:', () => {
    ...
    it('Should be failed when type of quantity is wrong', async () => {
      const res = await request(server)
        .put(`/v1/carts/invalid-cartId`)
        .set('Authorization', 'Bearer ' + token)
        .send({ quantity: [1, 2] });
