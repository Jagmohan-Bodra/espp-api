...
describe('Test DELETE //v1/product-price/:productId/:membershipId', () => {
  let token;
  let productId;
  let membershipId;
  ...
  describe('Return status 200', () => {
    it('Should be successful', async () => {
      const res = await request(server)
        .delete(`/v1/product-price/${productId}/${membershipId}`)
  ...
  describe('Return status 400', () => {
    it('Should response BadRequest when productId is invalid', async () => {
      const res = await request(server)
        .delete(`/v1/product-price/invalid-productId/${membershipId}`)
        .set('Authorization', 'Bearer ' + token);
    ...
    it('Should response BadRequest when membershipId is invalid', async () => {
      const res = await request(server)
        .delete(`/v1/product-price/${productId}/invalid-membershipId`)
        .set('Authorization', 'Bearer ' + token);
  ...
  describe('Return status 404', () => {
    it('Should failed when productId is not found', async () => {
      const res = await request(server)
        .delete(`/v1/product-price/123456789012/${membershipId}`)
        .set('Authorization', 'Bearer ' + token);
    ...
    it('Should failed when membershipId is not found', async () => {
      const res = await request(server)
        .delete(`/v1/product-price/${productId}/123456789012`)
        .set('Authorization', 'Bearer ' + token);
